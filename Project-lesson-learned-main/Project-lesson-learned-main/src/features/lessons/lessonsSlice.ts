...
export const addLesson = createAsyncThunk('lessons/addLesson', async (lesson: Omit<Lesson, 'id' | 'createdAt'>) => {
  const response = await api.post('/lessons', lesson);
  return response.data;
});

export const updateLesson = createAsyncThunk('lessons/updateLesson', async (lesson: Lesson) => {
  const response = await api.put(`/lessons/${lesson.id}`, lesson);
  return response.data;
});

export const deleteLesson = createAsyncThunk('lessons/deleteLesson', async (id: string) => {
