...
const ProjectPage = () => {
  ...
  const getStatusChip = (status: string) => {
    ...
    const statusConfig = statusMap[status as keyof typeof statusMap] || { color: 'default', icon: null };
  ...
  const getRiskChip = (riskLevel: string) => {
    ...
    const riskConfig = riskMap[riskLevel as keyof typeof riskMap] || { color: 'default', icon: null };
  ...
  const projectColumns: GridColDef[] = [
    ...
    { 
      ...
      getActions: (params) => [
        ...
        <IconButton 
          color="error"
          onClick={() => setProjects(projects.filter(project => project.id !== params.id))}
        >
          <DeleteIcon />
        </IconButton>,
        <IconButton 
          ...
          <Grid container spacing={3} sx={{ mb: 3 }}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Card>
                ...
                <CardContent>
                  <Stack spacing={1}>
                    {dashboardStats?.trendingCategories.map((category, index) => (
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
              ...
              <FormControl size="small" sx={{ minWidth: 180 }}>
                ...
                >
                  ...
                  {Array.from(new Set(lessons.map(lesson => lesson.category))).map(category => (
