...
const LessonDetailPage = () => {
  ...
  const lesson = {
    ...
    description: "Weekly status reports improved stakeholder engagement by 40% compared to bi-weekly reports. The more frequent communication helped identify issues earlier and kept stakeholders better informed.",
    ...
    impact: "positive",
    status: "approved",
    rootCause: "Previous bi-weekly reporting cadence was insufficient for the fast-paced nature of the project, leading to stakeholders feeling out of touch with progress and issues.",
    outcomes: "Implemented weekly reports with standardized format including: key accomplishments, upcoming milestones, risks/issues, and decisions needed. This reduced follow-up questions by 60% and improved stakeholder satisfaction scores.",
    suggestedActions: [
          ...
          <Paper sx={{ p: 3, mb: 3 }}>
            <Box sx={{ 
              ...
              <Typography variant="subtitle1" color="text.secondary">
                Project: <strong>{lesson.project}</strong> | Date Captured: <strong>{lesson.dateCaptured}</strong>
              </Typography>
              ...
              <Box sx={{ display: 'flex', gap: 1 }}>
                ...
                <Chip 
                  icon={getImpactIcon()}
                  label={lesson.impact === 'positive' ? 'Positive' : 
                         lesson.impact === 'negative' ? 'Negative' : 'Neutral'} 
                  color={lesson.impact === 'positive' ? 'success' : 
                         lesson.impact === 'negative' ? 'error' : 'warning'}
                  size="small"
                />
              </Box>
            </Box>

            <Tabs value={value} onChange={handleChange} aria-label="Lesson details tabs">
        ...
        <Grid item xs={12} md={4} component="div" {...({} as any)}>
          <Paper sx={{ p: 3, mb: 3 }}>
            ...
            <List dense>
              <ListItem>
                <ListItemAvatar>
                  <Avatar sx={{ bgcolor: theme.palette.grey[200], width: 32, height: 32 }}>
